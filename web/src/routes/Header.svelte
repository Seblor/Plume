<script>
  ...
  let menuOpen = false;
  $: if (pathname) menuOpen = false; // close on navigation
</script>
...
<header>
  ...
  <nav class="nav-desktop">
    ...
    <a
      class="nav-link"
      class:active={pathname.startsWith("/about")}
      href="/about">About</a
    >
  </nav>
...
<!-- Mobile dropdown menu — outside <header> so it doesn't change its height -->
{#if menuOpen}
...
<style>
  ...
  /* Mobile dropdown nav — positioned as a fixed overlay below header */
  .nav-mobile {
    display: none;
    position: fixed;
    top: 56px;
